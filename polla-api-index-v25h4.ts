...
async function setActivationCode(
  ...
  const codeHash = await hashPrivateValue(code.toUpperCase());
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (
      ...
      if (check.ok) {
        const session = await createAdminSession_(originHash);
        return jsonOut({
          ok: true,
    ...
    if (action === "authAdminConfirm") {
      ...
      const sessionCheck = await verifyAdminSession_(sessionToken, originHash);
      if (!sessionCheck.ok) {
      ...
      const pinCheck = await verifyAdminPin(params.pin, originHash);
      if (!pinCheck.ok) {
      ...
      const confirm = await createAdminConfirm_(purpose, originHash);
    ...
    if (
      ...
      let check: any = sessionToken
        ? await verifyAdminSession_(sessionToken, originHash)
        : await verifyAdminPin(params.pin, originHash);
      ...
      else if (action === "editPolla" && String(params.status || "") === "actual") {
    ...
    if (action === "getMatches") {
      ...
      const historicalStorageCleaned = !data.length && !!(await getCompactedHistory_(pollaId))?.storage_cleaned_at;
    ...
    if (action === "getParticipantsAdmin") {
      ...
      const { data = [], error } = await supabase
        ...
        .select(`id, jugador_id, paid, referral_code, referred_by_code, created_at,
                 jugadores(name, pin_hash, activation_code_hash, activation_purpose)`)
        .eq("polla_id", pollaId);
      if (error) throw error;
    ...
    if (action === "getMyParticipantStatus") {
      ...
      const part=await findParticipant(pollaId,jugador.id);
      if(!part) return jsonOut({ok:false,error:"NOT_A_PARTICIPANT"});
      return jsonOut({ok:true, paid:!!part.paid, referralCode:part.referral_code||"", referredByCode:part.referred_by_code||""});
    }
    ...
    if (action === "getStandings") {
      ...
      // F4: una Polla compactada conserva su tabla final dentro del snapshot.
      ...
      // C4: Postgres devuelve la tabla ya resumida. Evitamos traer a la Edge
      // Function todos los pronósticos resueltos para sumarlos en JavaScript.
      // Si la RPC todavía no existe, usamos el cálculo anterior como fallback.
      try {
        ...
        if (standingsRpcError) {
          console.warn("C4 standings RPC no disponible; usando fallback compatible", standingsRpcError.message || "");
        }
      } catch (_) {
        // fallback debajo
      }
    ...
    if (action === "selfResetPin") {
      ...
      if (!validPin(newPin)) return jsonOut({ ok:false, error:"PIN_INVALIDO" });
      const jugador = await findJugador(name);
      ...
      const rl = await getRateLimit("PIN_RECOVERY", targetKey, originHash);
      ...
      if (incoming !== jugador.security_answer_hash) {
        const f = await failRateLimit("PIN_RECOVERY", targetKey, originHash, 5, 5);
      ...
      await clearRateLimit("PIN_RECOVERY", targetKey, originHash);
    ...
    if (
      ...
      const [j,pollas,partidos,participantes,pronosticos,g,b,t,logs,refs,identityEvents,historyErasures] = await Promise.all([
        ...
        fetchAllPages<any>((from,to)=>supabase.from("player_history_erasure").select("*").range(from,to)).catch(()=>[]),
      ]);
      return jsonOut({
        version: 4,
    ...
    if (
      ...
      if (!lastSeason) {
        return jsonOut({
          ok: false,
          error:
            'Todavía no has cerrado ninguna temporada. Usa "🏁 Cerrar Temporada" antes de vaciar, o vas a borrar Pollas que nunca quedaron archivadas.',
        });
      }
      ...
      const {
        ...
        await supabase
          .from("pollas")
          .select("id, is_archived, season_started_at")
      ...
      // El cierre de temporada ya vació la base anual. Aquí NO borramos
      // admin_log ni tabla_acumulada_base para no destruir auditoría ni datos
      // de una nueva temporada si la limpieza se ejecuta días después.
    ...
    if (
      ...
      // Reabrir una Polla finalizada retira primero los campeones que esa
      // finalización aportó. Al finalizar otra vez se registran los campeones
      // resultantes, evitando títulos duplicados o desactualizados.
      if (reopeningFinal) {
        const previousStandings =
          await computePollaStandingsForAdmin(
            id,
          );

        if (
          previousStandings.length &&
          Number(
            previousStandings[0]
              .totalPoints,
          ) > 0
      ...
      if (becomingFinal) {
        ...
        if (pending.length) {
          return jsonOut({ ok:false, error:`No puedes finalizar: faltan ${pending.length} partido(s) por resolver o cancelar.`, pendingMatches:pending });
        }
        standingsFinal =
          await computePollaStandingsForAdmin(
            id,
        ...
        // En Postgres no hace falta comprimir/borrar filas:
        // archivamos congelando la Polla, pero los datos quedan íntegros.
        update.is_archived =
          true;
      }
    ...
    if (action === "getHistoricalPlayersAdmin") {
      const [players, erased] = await Promise.all([
        ...
        fetchAllPages<any>((from,to)=>supabase.from("player_history_erasure").select("player_id").range(from,to)).catch(()=>[]),
      ]);
    ...
    if (action === "previewHistoricalPlayerDeletion") {
      ...
      const [parts,preds,base,wins,seasons,histories]=await Promise.all([
        ...
        fetchAllPages<any>((from,to)=>supabase.from("polla_compacted_history").select("polla_id,snapshot_json").range(from,to)).catch(()=>[]),
      ]);
    ...
    if (
      ...
      if (!editable.ok) {
        return jsonOut({
          ok: false,
          error:
            "Esta Polla está archivada. Desarchívala primero para poder agregarle participantes.",
        });
      }
    ...
    if (
      ...
      if (!editable.ok) {
        return jsonOut({
          ok: false,
          error:
            "Esta Polla está archivada. Desarchívala primero para poder agregarle partidos.",
        });
      }
    ...
    if (
      ...
      if (!editable.ok) {
        return jsonOut({
          ok: false,
          error:
            "Esta Polla está archivada. Desarchívala primero para poder cancelar/reemplazar partidos.",
        });
      }
    ...
    if (
      ...
      const {
        ...
        await supabase
          .from("partidos")
          .select(
            "id, polla_id, is_star_match, is_canceled",
          )
          .eq("id", id)
          .maybeSingle();
